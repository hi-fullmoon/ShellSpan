...
use crate::agent_contract_v3::{
    AgentToolCallV3, AgentToolTargetV3, ApplyPatchArgumentsV3, FileEncodingV3,
    ListDirectoryArgumentsV3, ReadFileArgumentsV3, SearchModeV3, SearchTextArgumentsV3,
    TransferDirectionV3, TransferFileArgumentsV3,
};
...
use super::{
    connection_for_remote_target, sha256_hex, AgentFileCheckpointV3, CheckpointOriginalMetadataV3,
    CheckpointStoreV3, CheckpointTargetKindV3,
};
...
type ActiveFileOperationsV3 = Arc<Mutex<HashMap<(String, String), Arc<AtomicBool>>>>;

#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub(crate) struct AgentCallPreviewV3 {
...
pub(super) struct FileToolOutputV3 {
...
pub(crate) struct FileOperationRegistryV3 {
    operations: ActiveFileOperationsV3,
}

struct FileOperationGuardV3 {
    registry: FileOperationRegistryV3,
    task_id: String,
    call_id: String,
    flag: Arc<AtomicBool>,
}

impl FileOperationRegistryV3 {
    fn begin(&self, task_id: &str, call_id: &str) -> Result<FileOperationGuardV3, String> {
...
impl Drop for FileOperationGuardV3 {
    fn drop(&mut self) {
...
pub(super) struct FileExecutionContextV3<'a> {
    pub(super) task_id: &'a str,
    pub(super) call: &'a AgentToolCallV3,
    ...
    pub(super) checkpoints: &'a CheckpointStoreV3,
    pub(super) operations: &'a FileOperationRegistryV3,
}

pub(super) fn preview_file_call_v3(
    ...
    match call.tool_name.as_str() {
        "apply_patch" => {
            let arguments: ApplyPatchArgumentsV3 =
        ...
        "transfer_file" => {
            let arguments: TransferFileArgumentsV3 = serde_json::from_value(call.arguments.clone())
...
pub(super) fn execute_file_tool_v3(
    context: FileExecutionContextV3<'_>,
) -> Result<FileToolOutputV3, String> {
...
fn execute_read_file(
    context: &FileExecutionContextV3<'_>,
    operation: &FileOperationGuardV3,
) -> Result<FileToolOutputV3, String> {
    let arguments: ReadFileArgumentsV3 = serde_json::from_value(context.call.arguments.clone())
        .map_err(|error| format!("invalid read_file arguments: {error}"))?;
    let requested_limit = arguments
        .max_bytes
        .unwrap_or(DEFAULT_READ_BYTES)
        .min(1_048_576);
    let limit = if arguments.encoding == FileEncodingV3::Base64 {
    ...
    let content = match arguments.encoding {
        ...
        FileEncodingV3::Base64 => Some(STANDARD.encode(slice)),
        FileEncodingV3::MetadataOnly => None,
    };
...
fn execute_list_directory(
    ...
    let arguments: ListDirectoryArgumentsV3 =
...
fn execute_search_text(
    ...
    let arguments: SearchTextArgumentsV3 =
        serde_json::from_value(context.call.arguments.clone())
...
fn execute_transfer_file(
    ...
    let arguments: TransferFileArgumentsV3 = serde_json::from_value(context.call.arguments.clone())
        .map_err(|error| format!("invalid transfer_file arguments: {error}"))?;
...
fn validate_overwrite_precondition(
    arguments: &TransferFileArgumentsV3,
    existing: Option<&[u8]>,
) -> Result<(), String> {
    match existing {
        None => {
...
fn compute_patch_preview(
    ...
    arguments: &ApplyPatchArgumentsV3,
    database: &Database,
    credentials: &CredentialManager,
    known_hosts_path: &Path,
) -> Result<PatchPreviewV3, String> {
    ...
    let operation_registry = FileOperationRegistryV3::default();
...
fn checkpoint_target_file(
    ...
    let kind = match context.call.target {
        AgentToolTargetV3::Local { .. } => CheckpointTargetKindV3::Local,
        AgentToolTargetV3::Remote { .. } => CheckpointTargetKindV3::Remote,
...
fn read_target_file(
    ...
    operation: &FileOperationGuardV3,
    limit: u64,
) -> Result<(String, Vec<u8>, CheckpointOriginalMetadataV3), String> {
    match target {
        ...
        AgentToolTargetV3::Local { cwd: None, .. } => {
...
fn list_target_directory(
    ...
    known_hosts_path: &Path,
    operation: &FileOperationGuardV3,
) -> Result<(String, Vec<DirectoryEntryV3>), String> {
    operation.ensure_active()?;
    match target {
        AgentToolTargetV3::Local {
            cwd: Some(root), ..
...
fn search_target(
    target: &AgentToolTargetV3,
    arguments: &SearchTextArgumentsV3,
    database: &Database,
    credentials: &CredentialManager,
    known_hosts_path: &Path,
    operation: &FileOperationGuardV3,
) -> Result<(String, Vec<SearchMatchV3>, bool), String> {
    match target {
        AgentToolTargetV3::Local {
            cwd: Some(root), ..
        } => {
            ...
            for file in files.into_iter().take(MAX_SEARCH_FILES) {
                ...
                if matches!(arguments.mode, SearchModeV3::FileName | SearchModeV3::Both)
                    && contains_query(
                        file.file_name()
                ...
                if matches!(arguments.mode, SearchModeV3::Content | SearchModeV3::Both) {
...
fn local_metadata(path: &Path) -> Result<CheckpointOriginalMetadataV3, String> {
    ...
    Ok(CheckpointOriginalMetadataV3 {
        permissions,
        modified_unix_ms,
    })
}
...
pub(super) fn restore_local_checkpoint_v3(
    root: &Path,
    target_path: &str,
    original: Option<&[u8]>,
    ...
    let destination = resolve_local_destination(&root, target_path)?;
    let registry = FileOperationRegistryV3::default();
    let operation = registry.begin("restore", "restore")?;
    match original {
        Some(content) => {
            write_local_atomic(&root, &destination, content, current_digest, &operation)?;
            restore_local_metadata(&destination, metadata)
        }
        None => {
            let current = read_local_optional(&destination, MAX_TRANSFER_BYTES, &operation)?;
            if current_digest.is_some_and(|expected| {
                current.as_deref().map(sha256_hex).as_deref() != Some(expected)
            }) {
                return Err("restore target drifted".into());
            }
            fs::remove_file(destination)
                .map_err(|error| format!("failed to remove restored-created file: {error}"))
        }
    }
}

pub(super) fn restore_remote_checkpoint_v3(
    sftp: &Sftp,
    root: &str,
    target_path: &str,
    original: Option<&[u8]>,
    ...
    let root = resolve_remote_root(sftp, root)?;
    let destination = resolve_remote_path(sftp, &root, target_path, true)?;
    let registry = FileOperationRegistryV3::default();
    let operation = registry.begin("restore", "restore")?;
    match original {
        Some(content) => {
            ...
            restore_remote_metadata(sftp, &destination, metadata)
        }
        None => {
            let current = read_remote_optional(sftp, &destination, MAX_TRANSFER_BYTES, &operation)?;
            if current_digest.is_some_and(|expected| {
            ...
            sftp.unlink(Path::new(&destination))
                .map_err(|error| format!("failed to remove restored-created remote file: {error}"))
        }
    }
}

fn restore_local_metadata(
    path: &Path,
    metadata: &CheckpointOriginalMetadataV3,
) -> Result<(), String> {
    #[cfg(unix)]
    if let Some(mode) = metadata.permissions {
        ...
        fs::set_permissions(path, fs::Permissions::from_mode(mode))
            .map_err(|error| format!("failed to restore local permissions: {error}"))?;
    }
    if let Some(modified_unix_ms) = metadata.modified_unix_ms {
        let modified = std::time::UNIX_EPOCH
            .checked_add(std::time::Duration::from_millis(modified_unix_ms))
            .ok_or_else(|| "checkpoint modified time is invalid".to_string())?;
        File::options()
            .write(true)
            .open(path)
            .and_then(|file| file.set_times(fs::FileTimes::new().set_modified(modified)))
            .map_err(|error| format!("failed to restore local modified time: {error}"))?;
    }
    Ok(())
}

fn restore_remote_metadata(
    sftp: &Sftp,
    path: &str,
    metadata: &CheckpointOriginalMetadataV3,
) -> Result<(), String> {
    if metadata.permissions.is_none() && metadata.modified_unix_ms.is_none() {
        return Ok(());
    }
    let stat = ssh2::FileStat {
        ...
        atime: None,
        mtime: metadata.modified_unix_ms.map(|value| value / 1_000),
    };
    sftp.setstat(Path::new(path), stat)
        .map_err(|error| format!("failed to restore remote metadata: {error}"))
}

pub(super) fn current_local_digest_v3(
    root: &Path,
    target_path: &str,
    ...
    let destination = resolve_local_destination(&root, target_path)?;
    let registry = FileOperationRegistryV3::default();
    let operation = registry.begin("digest", "digest")?;
    read_local_optional(&destination, MAX_TRANSFER_BYTES, &operation)
        .map(|content| content.as_deref().map(sha256_hex))
}

pub(super) fn current_remote_digest_v3(
    sftp: &Sftp,
    root: &str,
    target_path: &str,
) -> Result<Option<String>, String> {
    ...
    let registry = FileOperationRegistryV3::default();
    let operation = registry.begin("digest", "digest")?;
    read_remote_optional(sftp, &destination, MAX_TRANSFER_BYTES, &operation)
        .map(|content| content.as_deref().map(sha256_hex))
}
...
mod tests {
    ...
    fn transfer_overwrite_requires_exact_destination_digest() {
        ...
        let base = TransferFileArgumentsV3 {
            direction: TransferDirectionV3::Upload,
