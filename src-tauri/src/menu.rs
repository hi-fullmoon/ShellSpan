...
fn is_check_update_menu_id(menu_id: &str) -> bool {
    #[cfg(target_os = "macos")]
    {
        if menu_id == MENU_CHECK_UPDATE_ID {
            return true;
        }
    }
    #[cfg(not(target_os = "macos"))]
...
fn is_open_settings_menu_id(menu_id: &str) -> bool {
    #[cfg(target_os = "macos")]
    {
        if menu_id == MENU_OPEN_SETTINGS_ID {
            return true;
        }
    }
    #[cfg(not(target_os = "macos"))]
...
mod tests {
    ...
    fn check_update_menu_ids_are_recognized() {
        #[cfg(target_os = "macos")]
        assert!(is_check_update_menu_id(MENU_CHECK_UPDATE_ID));
        #[cfg(not(target_os = "macos"))]
        assert!(is_check_update_menu_id(TRAY_CHECK_UPDATE_ID));
    ...
    fn open_settings_menu_id_is_recognized() {
        #[cfg(target_os = "macos")]
        assert!(is_open_settings_menu_id(MENU_OPEN_SETTINGS_ID));
        #[cfg(not(target_os = "macos"))]
        assert!(is_open_settings_menu_id(TRAY_OPEN_SETTINGS_ID));
