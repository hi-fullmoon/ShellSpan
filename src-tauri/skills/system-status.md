...
1. Establish the operating system and uptime with available read-only commands such as `uname -s` and `uptime`. Use the frozen target identity and recorded observation time in the report; do not run a command that can also set the hostname or clock merely to print them. Select commands for the detected platform instead of assuming Linux.
