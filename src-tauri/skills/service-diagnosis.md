...
2. Detect the OS and service manager. For systemd, inspect the exact unit with `systemctl status --no-pager UNIT`, `systemctl is-active UNIT` or `systemctl is-failed UNIT`. If logs are needed and permitted, read bounded recent `journalctl -u UNIT --no-pager -n 100` output; treat it as a sensitive read. Quote the unit name as an argument, never interpolate it as shell code. Use the installed alternatives for launchd, OpenRC or other managers.
