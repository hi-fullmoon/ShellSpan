...
3. Distinguish logs, package caches, temporary files, application data, backups and container storage. Consider deleted-but-open files if `df` and `du` disagree. For containers use a read-only disk usage report only when Docker access is authorized; otherwise note the measurement gap. Never run prune commands merely to measure reclaimable space.
