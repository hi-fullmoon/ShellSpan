...
- Deployment Runbook v2 已完成阶段 2/5：在阶段 1 纯语义契约、TypeScript/Rust 严格验证、共享夹具和规范化序列化之上，增加一次性精确审批、冻结目标、单主机制品/release/原子激活/systemd/健康验证状态机；仍不进入 v1 执行或多主机调度。
...
- Deployment Runbook v2 阶段 2 已具备受审阅的单主机制品传输、release/原子激活、受控 systemd、健康探测和审批状态机；阶段 3 仍需实现单独审批的声明式回滚执行。持久恢复、多主机、保留清理和专用 UI 继续 deferred，且不得把 v2 字段解释为任意 Shell。
