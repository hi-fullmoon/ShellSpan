...
- Deployment Runbook v2 已建立纯语义契约、TypeScript/Rust 严格验证、共享夹具和规范化序列化；当前仅为阶段 1/5 的契约能力，不进入 v1 执行或多主机调度。
...
- Deployment Runbook v2 尚无制品传输、release/原子激活、服务控制、健康探测、审批状态机或声明式回滚执行；不得把 v2 字段解释为任意 Shell。
