...
- Deployment Runbook v2 已完成阶段 4/5：在阶段 2 单主机执行与阶段 3 持久化/恢复/独立回滚边界之上，增加显式目标的 canary + rolling 批次、逐批人工审批、健康/失败阈值熔断、独立主机身份/取消/记录和禁止自动续跑的持久恢复；仍不进入 v1 tag 调度、流量切换或专用 UI。
...
- Deployment Runbook v2 阶段 4 已具备确定性的显式目标 canary/rolling 协调、逐批人工门、阈值熔断、持久批次/主机/审批/健康状态与新 review 恢复；专用 UI/模板（阶段 5）、流量/负载均衡集成、动态发现、无人值守回滚和实际清理执行继续 deferred，且不得把 v2 字段解释为任意 Shell。
