...
- Deployment Runbook v2 已完成阶段 3/5：在阶段 2 单主机正向部署之上，增加持久化审阅/审批消费/动作/健康/快照/终态、重启中断封存、独立回滚审阅与执行边界，以及保留清理候选和非执行式清理审阅；仍不进入 v1 执行或多主机调度。
...
- Deployment Runbook v2 阶段 3 已具备单独审批的声明式回滚、持久检查点、重启恢复封存、一次性快照占用、只读运行查询，以及引用安全的清理候选/审阅语义；多主机批次/金丝雀（阶段 4）、专用 UI（阶段 5）和实际清理执行继续 deferred，且不得把 v2 字段解释为任意 Shell。
