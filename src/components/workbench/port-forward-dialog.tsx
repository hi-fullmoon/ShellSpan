...
import { isPortForwardActive, type ManagedPortForwardRuntime, usePortForwardStore } from '@/stores/portForwardStore';
...
function latestRuntime(runtimes: ManagedPortForwardRuntime[], ruleId: string): ManagedPortForwardRuntime | undefined {
  const matching = runtimes.filter((runtime) => runtime.configId === ruleId);
  return matching.find(isPortForwardActive) ?? matching[0];
}

function statusVariant(status: ManagedPortForwardRuntime['status'] | undefined): React.ComponentProps<typeof Badge>['variant'] {
...
export function PortForwardDialog({ profile: requestedProfile, onClose }: PortForwardDialogProps): React.JSX.Element {
  const { t } = useI18n();
  const profile = useProfileStore((state) =>
    requestedProfile ? (state.profiles.find((item) => item.id === requestedProfile.id) ?? requestedProfile) : undefined,
  );
  const allRuntimes = usePortForwardStore((state) => state.runtimes);
  const runtimes = useMemo(() => (profile ? allRuntimes.filter((runtime) => runtime.profileId === profile.id) : []), [allRuntimes, profile]);
  ...
  const validationError = useMemo(() => {
    ...
    if (draft.kind === 'remote' && !['127.0.0.1', 'localhost', '::1'].includes(draft.remoteHost.trim()))
  ...
  const saveDraft = async (): Promise<void> => {
    ...
    const nextRules = draft.id ? rules.map((rule) => (rule.id === draft.id ? nextRule : rule)) : [...rules, nextRule];
    setSaving(true);
    try {
    ...
    <Dialog open={Boolean(profile)} onOpenChange={(open) => !open && onClose()}>
      <CompactDialogContent className="max-w-3xl">
        <CompactDialogHeader
          ...
          description={profile ? t('portForward.description', { name: profile.name }) : t('portForward.descriptionEmpty')}
        />
          ...
          {draft ? (
            <Card size="sm" className="shrink-0">
              ...
              <CardContent>
                <FieldGroup>
                  ...
                  <Field>
                    ...
                    <FieldDescription>
                      {draft.kind === 'local' ? t('portForward.kind.localDetail') : t('portForward.kind.remoteDetail')}
                    </FieldDescription>
                  </Field>
                  <div className="grid gap-4 sm:grid-cols-3">
                    <Field>
                      <FieldLabel htmlFor="port-forward-local-port">
                        {draft.kind === 'local' ? t('portForward.form.listenPort') : t('portForward.form.localTargetPort')}
                      </FieldLabel>
                      <Input
                    ...
                    <Field>
                      <FieldLabel htmlFor="port-forward-remote-host">
                        {draft.kind === 'local' ? t('portForward.form.remoteTargetHost') : t('portForward.form.remoteListenHost')}
                      </FieldLabel>
                      <Input
                    ...
                    <Field>
                      <FieldLabel htmlFor="port-forward-remote-port">
                        {draft.kind === 'local' ? t('portForward.form.remoteTargetPort') : t('portForward.form.remoteListenPort')}
                      </FieldLabel>
                      <Input
                  ...
                  <Field className="flex-row items-center justify-between rounded-lg border p-3">
                    <div>
                      <FieldLabel htmlFor="port-forward-auto-start">{t('portForward.form.autoStart')}</FieldLabel>
              ...
              <CardFooter className="justify-end gap-2">
                ...
                <Button size="sm" disabled={Boolean(validationError) || saving} onClick={() => void saveDraft()}>
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="text-sm text-muted-foreground">{t('portForward.summary', { configured: rules.length, active: activeCount })}</div>
                ...
                return (
                  <Card key={rule.id} size="sm" className="shrink-0">
                    <CardHeader>
                      ...
                      <CardAction>
                        <Badge variant={statusVariant(runtime?.status)}>{t(`portForward.status.${runtime?.status ?? 'idle'}`)}</Badge>
                      </CardAction>
                    </CardHeader>
                    <CardContent className="flex flex-col gap-3">
                      {runtime ? (
                        <div className="grid gap-2 text-xs text-muted-foreground sm:grid-cols-4">
                          <span>
                            {t('portForward.started')}: {runtime.startedAt ? new Date(runtime.startedAt).toLocaleString() : '—'}
                          </span>
                          <span>
                      ...
                      {runtime?.lastError ? (
                        <Alert variant="destructive">
                          <AlertCircleIcon />
                          <AlertTitle>
                            {runtime.errorCategory === 'portInUse' ? t('portForward.error.portInUseTitle') : t('portForward.error.title')}
                          </AlertTitle>
                    ...
                    <CardFooter className="flex-wrap justify-between gap-2">
                      <div className="flex gap-1">
                        <Button size="xs" variant="outline" onClick={() => setDraft(createDraft(rule))} disabled={active}>
                      ...
                      {active && runtime ? (
                        ...
                        <Button size="xs" onClick={() => profile && void usePortForwardStore.getState().startRule(profile, rule, 'manual')}>
        ...
        <CompactDialogFooter>
          {runtimes.some((runtime) => !isPortForwardActive(runtime)) ? (
            <Button variant="ghost" size="sm" onClick={() => profile && usePortForwardStore.getState().clearFinished(profile.id)}>
