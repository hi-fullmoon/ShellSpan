    ...
    <Dialog open={Boolean(profile)} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-h-[85vh] max-w-3xl overflow-y-auto">
        ...
        {draft ? (
          <Card>
            ...
            <CardFooter className="justify-end gap-2">
              ...
              <Button disabled={Boolean(validationError) || saving} onClick={() => void saveDraft()}>
                {t('common.save')}
              </Button>
            </CardFooter>
          </Card>
        ...
        {!draft && rules.length === 0 ? (
          <Alert>
            <CableIcon />
            <AlertTitle>{t('portForward.emptyTitle')}</AlertTitle>
            <AlertDescription>{t('portForward.emptyDescription')}</AlertDescription>
          </Alert>
        ) : null}
