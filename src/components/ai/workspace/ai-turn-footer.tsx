...
type FooterLabel = Extract<LocaleKey, `ai.workspace.turnFooter.${string}`> extends `ai.workspace.turnFooter.${infer Key}` ? Key : never;

function duration(milliseconds: number, t: Translate): string {
  const seconds = milliseconds < 10_000 ? Math.round(milliseconds / 100) / 10 : Math.round(milliseconds / 1_000);
  return seconds < 60
...
function StatDetail({ stat, label, value }: { readonly stat: string; readonly label: string; readonly value: ReactNode }) {
  if (value === null || value === undefined) return null;
  return (
    <div className="flex items-baseline justify-between gap-3" data-stat={stat}>
...
function StatPopover({
  ...
  return (
    <Popover.Root>
      <Popover.Trigger render={<Button type="button" variant="plain" size="sm" className="ai-turn-stat-trigger h-7 px-[7px]" />}>
        {icon}
        <span>{label}</span>
      </Popover.Trigger>
      <Popover.Portal>
        <Popover.Positioner side="top" align="start" sideOffset={8} collisionPadding={8} className="z-50">
          <Popover.Popup
            className="ai-turn-stat-panel max-h-[var(--available-height)] w-[300px] max-w-[calc(100vw-16px)] overflow-auto px-[15px] py-3"
            initialFocus={false}
          >
            <div className="ai-turn-stat-heading mb-2.5 flex items-center justify-between gap-3">
              <Popover.Title className="m-0 flex items-center gap-1">
                {icon}
                <span>{title}</span>
              </Popover.Title>
...
export function AiTurnFooter({ node }: { readonly node: AiConversationNodeOf<'turnTail'> }) {
  ...
  const providerName = (providerId: string): string =>
    routes?.find((route) => route.id === providerId)?.displayName.trim() ||
    providers.find((provider) => provider.id === providerId)?.name.trim() ||
    providerId;
  const label = (key: FooterLabel): string => t(`ai.workspace.turnFooter.${key}`);
  const tokens = (value: number | null): string | null => (value === null ? null : `${value.toLocaleString(locale)} tok`);
  const time = (value: number | null | undefined): string | null => (value == null ? null : duration(value, t));
  const totalTime = time(node.durationMs);
  const input =
    stats.uncachedInputTokens !== null && stats.cacheReadTokens !== null && stats.cacheWriteTokens !== null
      ? stats.uncachedInputTokens + stats.cacheReadTokens + stats.cacheWriteTokens
      : null;
  const cacheHit = input !== null && input > 0 && stats.cacheReadTokens !== null ? `${((stats.cacheReadTokens / input) * 100).toFixed(1)}%` : null;
  const hasUsage = [
    stats.totalTokens,
  ...
  const hasTiming = [node.durationMs, stats.modelDurationMs, stats.toolDurationMs, stats.averageTimeToFirstTokenMs, stats.tokensPerSecond].some(
    (value) => value != null,
  );
  ...
  return (
    <div
      className="ai-turn-tail min-w-0 max-w-full"
    ...
    >
      {node.status === 'incomplete' && (
        <Badge variant="secondary" className="mb-1">
          {t(stepBudgetReached ? 'ai.workspace.turnProcess.stepBudgetReached' : 'ai.workspace.turnProcess.incomplete')}
        </Badge>
      )}
      <MessageActions
          ...
          >
            <dl className="ai-turn-stat-details mt-2.5 mb-0.5 grid gap-[7px]">
              ...
              <StatDetail
                stat="rate"
                label={label('rate')}
                value={stats.tokensPerSecond === null ? null : `${Number(stats.tokensPerSecond.toFixed(1))} tok/s`}
              />
              <StatDetail
                stat="ttft"
