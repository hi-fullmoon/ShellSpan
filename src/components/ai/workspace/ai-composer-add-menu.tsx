...
export function AiComposerAddMenu({ disabled, agent, anchor, onAddFile, onAddFolder, onSkill,
  ...
  return <DropdownMenu open={open && !disabled} onOpenChange={setOpen}>
    ...
    <DropdownMenuContent side="top" sideOffset={8} align="start"
      ...
      className="ai-composer-add-menu flex h-[360px] max-h-(--available-height) min-h-0 w-(--anchor-width) max-w-(--available-width) flex-col overflow-hidden p-0 text-muted-foreground">
      <div className="min-h-0 flex-1 overflow-y-auto" data-composer-menu-scroll="">
        <div className="flex flex-col gap-1 p-2">
          <DropdownMenuGroup>
            ...
            <DropdownMenuItem className="min-h-7 gap-1.5" disabled={!agent} onClick={onAddFolder} aria-label={t('ai.workspace.attachments.folder')}
              aria-description={!agent ? t('ai.workspace.attachments.agentOnly') : t('ai.workspace.attachments.projectHint')}>
              <FolderPlusIcon /><span className="shrink-0">{t('ai.workspace.attachments.folder')}</span>
              <span className="truncate text-xs text-muted-foreground">{t(agent ? 'ai.workspace.addMenu.folderHint' : 'ai.workspace.addMenu.agentOnly')}</span>
            </DropdownMenuItem>
          </DropdownMenuGroup>
          <DropdownMenuGroup>
            <DropdownMenuLabel className="py-0.5">{t('ai.workspace.skills.title')}</DropdownMenuLabel>
            {builtinSkills.map(skill => <DropdownMenuItem key={skill.name} className="min-h-7 gap-1.5" disabled={!agent}
            ...
            {!agent && <p className="px-1.5 py-1 text-xs text-muted-foreground">{t('ai.workspace.addMenu.agentOnly')}</p>}
          </DropdownMenuGroup>
          <DropdownMenuGroup>
            <DropdownMenuLabel className="py-0.5">{t('ai.workspace.addMenu.history')}</DropdownMenuLabel>
            <p className="px-1.5 py-1 text-xs text-muted-foreground">{t('ai.workspace.addMenu.mentionSearchHint')}</p>
          </DropdownMenuGroup>
        </div>
      </div>
    </DropdownMenuContent>
