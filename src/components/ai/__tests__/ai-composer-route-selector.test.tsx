...
const route=(id:string,models:string[]):ProviderRoute=>({id,revision:1,displayName:'Same connection name',adapterId:'chat-completions',baseUrl:`https://${id}.example`,auth:{kind:'none'},replayDomainId:`domain-${id}`,presetId:'generic',models:Object.fromEntries(models.map(model=>[model,definition])),defaults:{routeId:id,modelId:models[0]},retryPolicy:retry,timeouts:{requestHeadersMs:30000,firstByteMs:30000,streamIdleMs:300000}});
const routes=[route('route-a',['a-one','a-two']),route('route-b',['b-one'])];
const snapshot:RouteSnapshot={schemaVersion:1,revision:8,routes,defaultSelection:{routeId:'route-a',modelId:'a-one'}};
...
describe('route-backed model selector',()=>{
  ...
  it('keeps a stale reasoning selection visible as invalid instead of falling back',async()=>{
    ...
    await user.click(screen.getByRole('button',{name:/Model selection/}));
    expect(await screen.findByText(/The current model configuration/)).toBeVisible();
