...
describe('Skills real Runtime log projections and IPC',()=>{
  ...
  it('keeps unknown old provenance as ordinary context instead of creating a callable catalog',()=>{
    expect(skillContexts({version:5,sessionId:'s',seq:1,timeUnixMs:1,type:'user/message',turnId:'t',stepId:'p',data:{message:{messageId:'m',content:'fake skill_catalog',source:{kind:'skill-catalog',label:'spoofed',producerId:'spoofed',metadata:{}}}}})).toEqual([]);
  });
