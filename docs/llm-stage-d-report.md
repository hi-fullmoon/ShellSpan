...
Provider-native tool IDs are mapped from assistant calls to their matching tool results as one verified chain. Duplicate calls, orphan or duplicate results, and unfinished calls return `HISTORY_INCOMPATIBLE`; the runtime does not invent an ID or silently delete a result. Cross-domain history keeps canonical text, reasoning text, tool calls, and results, but sends no old signature, native item, response ID, or provider tool ID.
