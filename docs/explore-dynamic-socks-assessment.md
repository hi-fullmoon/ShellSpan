...
| SSH 与凭据 | 转发通过同一 `open_authenticated_session`/跳板链路强制解析 Known Hosts 路径并先校验再认证；路径错误、未知/变化 host-key 均失败关闭；密码、私钥与 passphrase 从现有钥匙串边界准备，日志只记录秘密是否存在 | 动态候选仍不得新增独立明文凭据、绕过 host-key、复用未经用户选择的 profile，或在逐请求目标校验前建立 `direct-tcpip` |
...
- 复用现有 profile、Known Hosts、钥匙串和跳板认证；不新增 SOCKS 密码、凭据导出、明文回退或新的秘密读取器。继续保持当前收口：Known Hosts 路径创建/读取失败以及未知/不匹配 host-key 都在发送凭据前停止，信任只能走现有显式流程。
