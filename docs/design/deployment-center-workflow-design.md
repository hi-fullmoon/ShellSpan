...
部署中心主体采用连续工作区，不使用 Card 作为布局、分组或信息展示容器。工作流列表、工具栏、画布、配置区和底部状态区由相邻边框、`Separator`、可调整分隔手柄与各自的滚动层建立层级；同级区域之间不使用卡片间距制造分组。
...
- 左栏建议 224–260 px，右栏建议 300–360 px，中部占用剩余宽度；分隔手柄本身承担区域边界。
- 工作流列表在设计、准备发布、运行记录和版本视图中始终可达；节点库通过工具栏 Drawer 或左栏紧凑分段进入，不长期占据半个侧栏。
- 页面主体不拥有横向滚动。工作流列表、图画布和 Inspector 各自拥有滚动；固定工具栏、标题和底部状态区不随正文滚出视口。

中等容器与 AI 面板挤压：主区域只保留当前图或业务视图，工作流列表、节点库和配置区使用带固定标题的 Drawer。Header 操作区保持单行，搜索框允许收缩，不能因为 AI 面板改变 viewport 内的可用宽度而使用错误布局。

窄容器：不渲染需要横向拖动的无限画布，改为按拓扑顺序展示可编辑的紧凑节点行；相邻行使用 `Separator` 区分，不使用 Card。每行显示名称、执行域、状态、上下游数量和配置入口。连接操作使用“选择输出节点/端口”的可读 label 表单完成，并且能够完成添加、删除、连接、断开、参数修改、校验和保存。

响应式断点必须基于 `WorkbenchPage` container query，而不是只使用 viewport `sm/lg`。Header 保持默认高度；长配置面板和 Drawer 使用完整的 `min-h-0 flex-1` 收缩链，仅正文滚动。当前视觉回归基准以约 1418 px、858 px、428 px 和 AI 挤压后的 778 px 页面容器为代表宽度。

### 12.4 图投影与节点块

React Flow 只作为图的前端投影与交互层，不是新的领域模型：

- `DeploymentWorkflowDefinition.nodes[*].inputs` 是唯一连接真相，edge 每次由 input binding 投影，不单独持久化到组件、Store 或数据库；
- 节点位置只来自 `DeploymentWorkflowLayout.nodes`；移动仅产生 layout dirty，连接、断开、增删节点和参数修改产生 semantic dirty；
- viewport 是个人 UI 状态，不进入语义定义，也不因缩放、平移或 Fit View 触发保存语义；
- native validator 和 compiler 继续是最终权威，画布只提供即时反馈；运行仍只消费 compiler 生成的不可变计划。

设计态节点是紧凑的图节点块，不是 Card：无阴影和抬升层级，使用细边框、轻量圆角和紧凑内边距，并明确显示输入、输出 Handle。节点块显示：
...
选中、校验错误和只读状态必须同时使用边框、图形或文字语义，不能只依赖颜色。键盘能够聚焦和选择节点，并有完整的移动、连接和断开替代路径。

### 12.5 运行视图

宽容器使用“运行列表 / 只读 DAG / 节点详情与时间线”连续三栏。运行图复用设计态的 binding 投影和节点骨架，但严格禁用移动、连接、重连和删除；中窄容器把节点详情切换到 Drawer，并让运行列表位于顶部或独立 Drawer。运行节点显示：
...
运行 DAG 的节点状态只来自 `DeploymentRunNodeRecord` 等 native 投影，不从图结构推断调度结果。`state_unknown`、运行中和等待审批等持续状态继续使用上下文 Alert；attempt、receipt、evidence 和 Artifact 入口进入节点 Inspector。
...
- frontend：可读 label、节点连接、语义区域与工具栏、宽/中/窄响应式结构、滚动归属、焦点返回和 Toast 去重测试；
