...
1. 打开一个已连接的本地或远程终端。
2. 打开 AI 面板。
3. 在 Composer 的权限菜单选择权限模式，在模型菜单选择已配置的模型和推理强度。
4. 输入目标并发送。
...
对话开始后仍可切换模型、推理强度和权限。选择只影响当前会话，并随会话日志保存；
不会修改其他会话或默认模型。运行中的模型请求及其重试保留原配置，下一个 Step 使用新模型。
权限变更影响之后准备的工具调用；已经等待审批的调用仍需处理原审批，已执行的调用不会被撤回。
切换失败时保留原选择并显示错误。结束、归档的会话和子 Agent 不提供独立切换。
...
新会话的权限预选只绑定当前连接实例，断线、关闭、重连、身份变化或应用重启后恢复为低风险只读模式。
已创建会话使用日志中记录的权限模式；恢复会话不会被新的预选覆盖。
