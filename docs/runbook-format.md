...
`schemaVersion: 2` with `kind: "deployment"` is the separate [Deployment Runbook v2 contract](deployment-runbook-v2.md). It adds semantic artifact/release/service/health/rollback metadata, a reviewed single-host executor, durable rollback/recovery, and a phase 4 explicit-target canary/rolling coordinator, but is not accepted by the v1 editor, tag scheduler, or execution parser. V2 fields are never inferred as Shell commands.
