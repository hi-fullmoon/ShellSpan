...
`schemaVersion: 2` with `kind: "deployment"` is the separate [Deployment Runbook v2 contract](deployment-runbook-v2.md). It adds semantic artifact/release/service/health/rollback metadata and a phase 2, reviewed single-host executor, but is not accepted by the v1 editor, scheduler, or execution parser. V2 fields are never inferred as Shell commands.
...
Deployment Runbook v2 remains outside this v1 executable audit path. The existing `execute_runbook_step` command continues to parse `schemaVersion: 1` exclusively, so a v2 deployment document fails there before action selection, credential resolution, or network access. Phase 2 instead uses separate review/execute/cancel commands that revalidate the canonical v2 document, one-time approval, frozen target, semantic plan, paths, digests, risk, and timeouts before running fixed deployment primitives. It exposes neither arbitrary Shell nor general SFTP, and it records rollback inputs without executing rollback.
