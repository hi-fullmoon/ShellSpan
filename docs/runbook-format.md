...
`schemaVersion: 2` with `kind: "deployment"` is the separate [Deployment Runbook v2 contract](deployment-runbook-v2.md). It adds semantic artifact/release/service/health/rollback metadata, a reviewed single-host executor, durable rollback/recovery, a phase 4 explicit-target canary/rolling coordinator, and the phase 5 [Deployment workbench](deployment-workflow.md), but is not accepted by the v1 editor, tag scheduler, or execution parser. V2 fields are never inferred as Shell commands.
...
Deployment Runbook v2 remains outside this v1 executable audit path. The existing `execute_runbook_step` command continues to parse `schemaVersion: 1` exclusively, so a v2 deployment document fails there before action selection, credential resolution, or network access. The deployment commands revalidate the canonical v2 document, one-time approval, frozen target, semantic plan, paths, digests, risk, and timeouts before running fixed deployment primitives. Phase 4's rollout commands accept only explicit ordered profiles and delegate every target to that same boundary; they expose neither arbitrary Shell nor general SFTP and never execute rollback automatically.
