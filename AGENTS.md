...
- 所有 Modal 弹框（包括部署中心）必须沿用系统共享 Dialog / AlertDialog 的视觉规范。底部操作区不添加分隔线，不通过 Footer 的 `border-t`、正文末尾的 `border-b` 或额外 `Separator` 实现底部分隔；正文与操作区使用间距区分。固定底部操作区继续保留 `shrink-0` 和正文独立滚动结构，新增或修改弹框时检查与系统现有弹框的一致性。
...
- 提交前删除临时调试输出、变量或对象转储、逐帧或逐条进度输出，以及重复的成功提示。仅用于开发时观察尺寸、状态或执行路径的日志属于非必要日志，不应保留在业务代码或测试脚本中。
- 保留有明确排障或审计用途的错误、警告与关键操作日志，沿用所在模块已有的日志设施、级别和脱敏方式。同一错误避免在多个调用层重复记录，不得为了通过提交检查删除必要日志。
- 测试以断言和退出码表达结果，优先使用测试框架的报告。独立验证脚本可以保留必要的最终摘要、失败诊断和长时间运行的进度提示；不要为每个断言、窗口尺寸或循环步骤输出成功日志。诊断信息优先放入断言消息，详细过程仅在显式启用调试或详细输出时记录。
- 不得仅为规避检查将非必要日志改写为其他输出 API，或添加无效的 lint 忽略注释。检查误报时应核实日志用途和钩子的实际规则；跳过检查不作为日常提交方式。
