...
| Local and remote rollout | **IMPLEMENTED; REMOTE DEFAULT OFF** | `SHELLSPAN_TERMINAL_INTERACTIVE_TOOLS_V1` requires Broker plus shell integration; remote publication additionally requires both the dedicated Agent SSH PTY flag and the independently default-off `SHELLSPAN_TERMINAL_REMOTE_INTERACTIVE_TOOLS_V1`. Disabling the applicable interactive flag revokes interactive leases without disabling remote visible commands. |
