...
| Compatibility execution removal | **PASS** | Routing returns only `TerminalExecute` or `Unavailable`. The `pty` channel, wrapper/parser implementation, fallback flag, and synthetic `[Agent]` echo are removed. |
...
| Persisted session compatibility | **PASS** | Existing `direct` / `boundTerminal` event-v5 values remain decodable without rewrite. New `exec_command` calls accept only `direct`; no operation is rerouted or replayed across paths. |
