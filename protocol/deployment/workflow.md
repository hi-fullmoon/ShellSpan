...
- `source.snapshot`: `{ sourceRef: 'workspace', binding? }`. New native preparations require an explicit local `binding`: `{ id, revision, localPath, repositoryIdentity, includedUntracked, excludedPaths? }`. `inspect_deployment_source_binding(localPath)` returns the canonical Git root, repository identity, HEAD and the untracked selection candidates without executing project scripts. Persist the returned binding through the existing workflow update transaction; changes create a new workflow revision. Historical unbound definitions remain readable, but cannot prepare a new native source snapshot. Repository identity hashes the Git history roots and object format, independently of the local path and without storing credential-bearing remote URLs. Shallow repositories are rejected until history is complete. Relocation requires identity verification and a new binding revision, without changing historical runs. Exclusions are explicit root-relative file/directory paths, are frozen with the binding, and are applied before reading members; they are suitable for data or repository tooling that must not enter a build.
...
- `artifact.bundle-compose`: `{ composeFiles, projectName, services, registeredMounts?, nonSensitiveFiles? }`; 1–8 source-relative files in explicit merge order. `registeredMounts` entries are `{ source, target, readOnly }` with absolute paths. `nonSensitiveFiles` explicitly declares source-relative config dependencies safe for the ordinary bundle.
...
Native executor `docker-compose/2` freezes the binding alongside the source snapshot and hashes actual file contents and executable bits. Tracked files and only explicitly selected, nonignored untracked files enter a private snapshot directory; symlinks and submodules are rejected. Each build receives a separate materialization, while Compose reads the unchanged captured source. Later workspace edits cannot replace the candidate. Preparation directories are released with the executor; after an interrupted preparation, prepare again. Fully prepared bundles remain in the existing CAS and do not require the local source during activation or reconciliation.

Executor version 1 remains accepted only for reconciliation of existing evidence and cancellation compensation, whose ledger/receipt contract is unchanged. New approval/start operations still require version 2; upgrading cannot silently execute an old pending candidate. The optional frozen source binding is omitted when absent so historical plan digests remain unchanged.

Compose YAML is decoded with serde_yaml and normalized/merged with the official `docker compose config` CLI. The first implementation supports exactly one managed service/image, removes its build section, replaces its image with the archive's unique reference and forces `pull_policy: never`. The archive image ID is retained and checked during loading. Activation must match the bundled project and service scope and use `pullPolicy: never`. Additional services/images, interpolation, implicit host environment, secrets, env_file, external configs, includes/extends, unsupported privileges, Docker socket/root mounts and unregistered binds block preparation. Declared non-sensitive config files are copied into the same immutable bundle and rewritten to bundle-local references. Published ports require explicit loopback bindings. No remote Compose build or implicit pull is authorized.

`imageId` is the verified config digest from the Docker archive. OCI archives additionally freeze `imageManifestId`, whose manifest/index bytes are digest-verified and resolve to exactly one runnable image with that config (bounded attestation descriptors are allowed). A loaded image must match one of these exact frozen identities, supporting both classic and containerd image stores without trusting tag equality alone. Registered binds use `create_host_path: false`; missing data directories must be explicitly initialized before activation.
...
## 17. Application and environment onboarding

When managed settings drift, the configuration dialog can explicitly load the workflow settings for review. Saving with `reconcileManagedFields: true` acknowledges those changes for the selected environment only. The normal save and CLI import default to false. Reconciliation retains revision checks, repository validation, unique-role mapping and the drift guard for other environments; it creates new immutable revisions and invalidates old readiness reports without preparing or starting a deployment.

Database migration 11 adds revisioned applications, source bindings, environments and read-only readiness reports. An environment links to at most one workflow; incomplete drafts may have no workflow. `applicationBinding` is an optional schema-v3 definition field containing application, environment and source IDs and revisions. Missing bindings keep historical definitions readable; new deployment preparation requires an explicit application association.

`inspect_deployment_project` reads Git identity, changed/untracked paths and bounded configuration files without running project scripts. Untracked paths must be selected explicitly. `save_deployment_application` validates the source repository boundary and uses expected application, source, environment and workflow revisions in one SQLite transaction. It updates unique managed node roles while retaining custom nodes, graph bindings, outputs and immutable history. Unsupported mappings and managed fields changed in the advanced editor fail closed without overwriting them. Other environments sharing the source receive matching workflow revisions atomically; drift in those workflows also aborts the transaction.

`preview_deployment_files` returns exact proposed files, existence flags and a digest. `apply_deployment_files` requires that digest, recomputes the preview and exclusively creates files; existing files are never overwritten. Generated files remain untracked until the user explicitly includes or commits them. Neither operation runs dependency installation or a build.

`check_deployment_readiness` runs local capture/tool/configuration checks and optionally fixed, audited SSH/SFTP read checks. It never initializes directories, installs dependencies, stops services or deploys. Reports distinguish passed, blocked, notice and unchecked items and preserve location, time and diagnostic evidence. SSH-user permissions do not establish container-user access: registered data mounts show a notice requiring the mandatory approved activation probe described below. Unknown existing services remain blocked and require manual migration.

Reports are tied to the exact environment/source/workflow revisions and configuration digest, expire after 15 minutes for preparation, and cannot be submitted directly by the frontend. Saving or changing relevant workflow definitions invalidates previous reports. New deploy preparation rejects missing, stale, blocked or unchecked reports; this gate does not replace frozen target checks, artifact validation, manual approval or execution-time reconciliation. Cancel/recovery/history remain available. Public ingress uses normal certificate verification. An absent optional public URL is a notice and permits server-local verification only. A configured URL must be reachable with trusted TLS; an HTTP response alone does not establish application availability. Onboarding completion is not evidence of a successful release.

## 18. Application release review and observations

The existing workflow page prepares, reviews and executes through the coordinator. Source, data and readiness configuration opens from workflow settings; workflow, run and version tabs remain the main interface. Preparation runs local build scripts, reports progress keyed by workflow and run, and supports cancellation. Review projects immutable `releaseReview` configuration, frozen target display, source revision and `changedFiles`, artifact digests, effects, HTTP scope, recovery policy and expiry. Subsequent source edits do not replace the candidate; workflow drift or expiry requires new preparation.

Initial Compose preflight checks the frozen project's existing containers and all published ports from the verified CAS Compose component using fixed read-only commands. Execution rechecks after acquiring the endpoint lock and before remote effects. Approval, start and lock-wait revalidation use the same real-clock expiry guard. Reconciliation uses that endpoint lock too. An unresolved run on the endpoint blocks new effects. Existing managed Compose services require locally recorded successful release evidence, the committed Compose digest, matching marker and actual container identity; an obsolete creation-time `config_files` label is not a restoration path.

## 19. Persistent data, historical rollback and retention

Registered bind mounts require explicit numeric UID:GID. Paths inside a deployment root must be under `shared`; explicitly registered external paths are permitted. Activation uses the frozen image and identity in a network-disabled, read-only-root temporary container. It checks directory read/traversal access and, for writable mounts, creates, reads and removes only its unique temporary probe directory. Failure prevents Compose activation. Read-only readiness performs no such writes. Generated Dockerfiles grant the declared identity access to application files and use that identity at runtime.

Historical rollback skips current source capture, builds and artifact bundling, uses the selected verified bundle, and retains normal target preflight, fresh approval and execution gates. Failed Compose activation verification restores only the frozen previous release's committed, digest-verified files and project. Automatic restoration requires unchanged mounts and container identity and rechecks the previous release's HTTP configuration. The failed run remains failed even when restoration succeeds. Application-level data migrations are outside this contract; files are never restored from a release bundle.

An absent remote effect receipt yields `stateUnknown`, never permission to replay. Artifact retention protects the configured number of distinct successful releases in addition to active leases, unresolved runs, time retention and current/previous references. Local artifact cleanup cannot target remote shared data. There is no automatic remote release-directory or shared-data deletion.

`observe_deployment_service` accepts a run ID, validates its immutable plan and current managed release, and invokes only its frozen target preflight and HTTP verification nodes. It cannot admit transfer, activation, commit or compensation. Results are persisted as dated `serviceObservation` events and returned in run detail. Successful publication also records its actual verification evidence. Observations never change the historical release outcome or claim continuous uptime; public ingress availability remains explicitly unverified. Read-only HTTP failures are definite check failures eligible for bounded retry; uncertain write outcomes retain reconciliation semantics.

## 19. Existing Compose projects

Executor version `docker-compose/3` adds optional `hostCompose` to `artifact.bundle-compose`. Absent configuration retains the isolated single-service contract. The application configuration exposes `hostCompose`, `gitRef` and optional `verification` (`packageManager: npm|pnpm`, `script`). Non-workspace source references resolve locally to commits and are captured from temporary detached worktrees; the user's checkout and uncommitted files remain untouched. Verification installs locked dependencies and runs the selected package script in a separate source copy before Docker building.

`hostCompose` contains the server-relative `environmentFile`, required `overrideFiles`, `recreateServices`, a `backup` script path and argument array, and HTTP `checks` (URL, status, optional exact redirect `location`, optional top-level string `jsonFields`). Host mode requires a committed source and `automaticRestore: false`. The bundle contains original Compose files, explicitly declared non-sensitive files, an immutable image override, a manifest, and the fixed Python 3 executor. It never includes server environment values. At most 60 deployment files are accepted. Credential files, host environment/override files, TLS/License/well-known paths and symbolic links cannot be replaced.

Read-only preflight validates the existing Compose project and captures a digest of relevant server files and container identities. Pre-start revalidation must match the frozen target digest. Activation verifies component checksums, holds a server-side lock, validates prospective Compose configuration, archives replaced files and previous image identities, and runs the selected server backup program without capturing its output. The backup program must return zero only after validating backup contents. No file replacement or service restart happens on backup failure. Only selected services are updated with `--no-deps --no-build --pull never`; specified companion services use `--force-recreate`. The last override fixes the loaded image without editing the environment file.

Activation checks exact running image identity, running/healthy selected containers and configured HTTP/JSON/redirect assertions with standard certificate validation. HTTP bodies and resolved Compose configuration never leave the remote executor. Missing effect receipts after interruption remain unknown. Configuration archives live under the release's `host-backup` directory (`files`, `recovery.json`, optional `previous-image.yaml`); failed attempts retain the staging archive. Database restoration is always a separate reviewed action. The existing isolated-release compensation command is forbidden for host mode. Preparing a rollback requires the stored host manifest to match the currently reviewed project, services, selected file paths and host configuration; configuration drift blocks reuse of an older artifact.

`ShellSpan --import-deployment <existing-database> <configuration.json>` imports a local application using the same validators and persistence path. A package contains `entry` and may contain a new key-based `connection`; an explicit local private-key path is imported into the system credential store, never the database or output. New imports reject duplicate application names and profile IDs. Updates require matching entry revisions and `expectedWorkflowRevision`, and cannot replace connection credentials. Import respects the admission gate and never performs preparation or deployment. `ShellSpan --check-deployment <existing-database> <application-id>` runs and stores the normal read-only readiness checks, using the existing credential store and trusted-host file.
